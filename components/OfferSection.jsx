      ...
      <section className="py-14 bg-white ">
        <div className="max-w-screen-xl mx-auto px-4 text-black md:px-8 pt-5 ">
          <div className="max-w-xl space-y-3 pt-3">
            <h1 className="text-black text-5xl font-semibold sm:text-4xl">What makes Chemisphere better?</h1>
          </div>
          <div className="mt-12">
            <ul className="grid gap-y-8 gap-x-12 sm:grid-cols-2 lg:grid-cols-3">
              {features.map((item, idx) => (
                <li key={idx} className="space-y-3 p-4 rounded-md neumorphic-card">
                  <div className="w-12 h-12 bg-white border-chemisphere border-2 text-white rounded-full flex items-center justify-center">
                    <Image src={item.image} alt={`tick_${idx + 1}`} width={30} height={30} />
                  </div>
                  <h4 className="text-lg text-black font-semibold">{item.title}</h4>
