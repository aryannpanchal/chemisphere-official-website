...
export default function DashboardForm() {
  ...
  const next = async () => {
    ...
    if (valid && step <= questions.length - 1) setStep((prev) => prev + 1);
  };
  ...
  >
    ...
    {current.type === 'select' ? (
      ...
      <textarea
        ...
        rows={4}
        className="w-full px-4 py-2 border border-gray-300 rounded-xl bg-white focus:outline-none focus:ring-2 focus:ring-outline-none"
      />
    ) : (
      <input
        type={current.type}
    ...
    {errors[current.name] && (
      <p className="text-red-500 text-sm">{errors[current.name].message || 'This field is required'}</p>
    )}
  </motion.div>
)}
